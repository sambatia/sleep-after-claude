...
prompt_and_handle_blockers() {
  ...
  if [[ ${#user_blockers[@]} -gt 0 ]]; then
    echo -e "    ${CYAN}[t]${RESET} Attempt to terminate the ${#user_blockers[@]} user app(s) listed above ${DIM}(recommended)${RESET}"
  fi
  echo -e "    ${CYAN}[s]${RESET} Skip — proceed with watch; sleep may not succeed"
  echo -e "    ${CYAN}[a]${RESET} Abort — I'll handle these manually"
  echo ""
  local choice
  printf "  Choice [%st/s/a]: " ""
...
check_for_update() {
  ...
  # blocks for long on a slow network.
  local tmp_remote
  tmp_remote="$(mktemp -t sac-update-check.XXXXXX 2>/dev/null)" || return 0
  if ! curl -fsSL --max-time 5 "$UPDATE_CHECK_URL" -o "$tmp_remote" 2>/dev/null; then
    rm -f "$tmp_remote"
    return 0
  fi
  ...
  if prompt_confirm "${BOLD}Update now?${RESET} [y/N]:"; then
    echo ""
...
if [[ "$SKIP_PREFLIGHT" == false ]]; then
  ...
  if [[ "$PREFLIGHT_SCAN_OK" != true ]]; then
    ...
    if [[ "$FORCE" == false ]]; then
      if [[ "$STDIN_IS_TTY" == true ]]; then
        if ! prompt_confirm "${BOLD}Sleep-blocker scan failed. Proceed anyway?${RESET} [y/N]:"; then
